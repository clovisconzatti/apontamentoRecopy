...
@section('content')
    <table class="table table-borderless table-advance table-condensed">
        <tr>
            <td width="80%">
                <h3>
                    <i class="fas fa-laptop"></i> Consumo de Insumo
                </h3>
            </td>
            <td width="50%" align="center">
                <h3>
                    <a class="cor-digiliza" href="{{route('insumo.add')}}">
    ...
    <div class="collapse" id="collapseExample">
        <div class="card card-body">
            <form method="get" action="{{ route('insumo.listAll') }}">
                @csrf
    ...
    <table class="table table-bordered table-condensed table-striped fonte-20">
        ...
        <tbody>
            @foreach ($insumos as $insumo)
                <tr>
                    <td align="center"> {{ date('d/m/Y',strtotime ($insumo->data)) }} </td>
                    <td align="right"> {{ $insumo->os }} </td>
                    <td> {{ $insumo->materiaprima }} </td>
                    <td align="right"> {{ $insumo->qtd }} </td>
                    <td>
                        <div class="btn-group-vertical">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="{{route('insumo.formEdit', $insumo->id)}}">
                                    <i class="far fa-edit"></i>&nbsp;&nbsp;&nbsp;
                                    <span>Editar</span>
                                </a>
                                <a class="dropdown-item" href="#">
                                    {{-- <form action=" {{ route('insumo.destroy',['insumo'=> $insumo->id ]) }} " method="POST">
                                        @csrf
                                        @method('delete')
